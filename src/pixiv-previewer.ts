...
let g_pageType = -1
// 图片详情页的链接，使用时替换 #id#
const g_artworkUrl = '/artworks/#id#'
// 获取图片链接的链接
const g_getArtworkUrl = '/ajax/illust/#id#/pages'
// 获取动图下载链接的链接
const g_getUgoiraUrl = '/ajax/illust/#id#/ugoira_meta'
// 获取小说列表的链接
const g_getNovelUrl = '/ajax/search/novels/#key#?word=#key#&p=#page#'
// 鼠标位置
let g_mousePos = { x: 0, y: 0 }
// 加载中图片
const g_loadingImage =
...
let g_settings
// 日志等级
const g_logLevel = LogLevel.Error
...
function PixivPreview() {
  ...
  function ActivePreview() {
    ...
    $(returnMap.controlElements).mouseenter(function (e) {
      ...
      const delay = parseInt(
        g_settings.previewDelay == null
          ? g_defaultSettings.previewDelay
          : g_settings.previewDelay
      )
      ...
      const previewDiv = $(document.createElement('div'))
        ...
        .css({
          ...
          height: '48px',
          'background-image':
            'url(https://pp-1252089172.cos.ap-chengdu.myqcloud.com/transparent.png)',
          display: 'none',
          'text-align': 'center',
        })
      // 添加到 body
      ...
      if (!g_settings.previewByKey) {
        const waitTime = delay - (new Date().getTime() - startTime)
        if (waitTime > 0) {
    ...
    $(returnMap.controlElements).mouseleave(function (e) {
      ...
      while (true) {
        if (
          moveToElement.hasClass('pp-main') &&
          moveToElement.attr('illustId') == illustId
        ) {
    ...
    $(returnMap.controlElements).mousemove(function (e) {
      ...
      if (
        !(g_settings.previewByKey && $('.pp-main').css('display') != 'none')
      ) {
        AdjustDivPosition()
      }
    })
  ...
  function AdjustDivPosition() {
    ...
    if (width > 0 && height > 0) {
      newWidth = isShowOnLeft
        ? g_mousePos.x - fromMouseToDiv
        : screenWidth - g_mousePos.x - fromMouseToDiv
      newHeight = (height / width) * newWidth
      // 高度不足以完整显示，只能让两侧留空了
      if (newHeight > screenHeight) {
        newHeight = screenHeight
    ...
    // 调整DIV的位置
    left = isShowOnLeft
      ? g_mousePos.x - newWidth - fromMouseToDiv
      : g_mousePos.x + fromMouseToDiv
    ...
    if (
      ...
      $('.pp-image')
        .on('load', function () {
          ...
          const _this = $(this)
          const size = AdjustDivPosition()
          const isShowOriginal = _this.hasClass('original')
  ...
  function ProcessAutoLoad() {
    ...
    if (newReturnMap.loadingComplete) {
      if (
        oldReturnMap.controlElements.length !=
          newReturnMap.controlElements.length ||
        newReturnMap.forceUpdate
      ) {
        DoLog(
          LogLevel.Info,
          'Page loaded ' +
